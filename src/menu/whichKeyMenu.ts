...
class WhichKeyMenu extends BaseWhichKeyMenu<BindingItem> {
  ...
  protected override async handleAccept(item: BindingItem): Promise<OptionalWhichKeyMenuState> {
  ...
  protected override async handleMismatch(key: string): Promise<OptionalWhichKeyMenuState> {
  ...
  protected override handleRender(items: BindingItem[]): BaseWhichKeyQuickPickItem<BindingItem>[] {
    ...
    const max = items.reduce((acc, val) => (acc > val.key.length ? acc : val.key.length), 0);

    return items.map((i) => {
      const icon = this.showIcons && i.icon && i.icon.length > 0 ? `$(${i.icon})   ` : "";
